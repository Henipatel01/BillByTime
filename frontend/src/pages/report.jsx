    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-4 py-3 text-sm">
      ...
      {payload.map((p) => (
        <p key={p.name} style={{ color: p.color }} className="font-semibold">
          {p.name}: {p.name === "Earnings (₹)" ? `₹${Number(p.value).toFixed(2)}` : p.value}
        </p>
      ))}
    </div>
  );
...
export default function Reports() {
  ...
  <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-950 tracking-tight">Reports</h1>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            ...
            <span className="ml-3 text-sm text-gray-500">Loading report...</span>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-8">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <div className="flex items-center justify-between mb-5">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-medium mt-0.5">Last {range} days</p>
                  </div>
                  <span className="text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-100 px-2.5 py-1 rounded-lg">
                    ₹{invoices.reduce((s, i) => s + (i.amount || 0), 0).toFixed(2)} total
                  </span>
                </div>
                {chartData.length === 0 ? (
                    ...
                    <AreaChart data={chartData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                      <defs>
                        <linearGradient id="earningsGrad" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="5%" stopColor="#1a4a3a" stopOpacity={0.15} />
                          <stop offset="95%" stopColor="#1a4a3a" stopOpacity={0} />
                        </linearGradient>
                      </defs>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <h2 className="text-sm font-bold text-gray-900">Clients Billed</h2>
                    <p className="text-xs text-gray-400 font-medium mt-0.5">Last {range} days</p>
                  </div>
                  <span className="text-xs font-bold text-blue-700 bg-blue-50 border border-blue-100 px-2.5 py-1 rounded-lg">
                    {new Set(invoices.map(i => i.email).filter(Boolean)).size} unique
                  </span>
                </div>
                {chartData.length === 0 ? (
                  <div className="flex items-center justify-center h-48 text-gray-300 text-sm">No data yet</div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100">
                  <h2 className="text-sm font-bold text-gray-900">Revenue by Pricing Type</h2>
                  <p className="text-xs text-gray-400 font-medium mt-0.5">All time breakdown</p>
                </div>
                {typeBreakdown.length === 0 ? (
                  <div className="text-center py-10 text-gray-300 text-sm">No data yet</div>
                ) : (
                  <div className="divide-y divide-gray-50">
                    {typeBreakdown.map((t) => {
                      const totalEarnings = typeBreakdown.reduce((s, x) => s + x.total, 0);
                      const pct = totalEarnings > 0 ? Math.round((t.total / totalEarnings) * 100) : 0;
                      return (
                        <div key={t.type} className="px-5 py-3.5">
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-400 font-medium">{t.count} invoice{t.count !== 1 ? "s" : ""}</span>
                            </div>
                          ...
                          <p className="text-xs text-gray-400 mt-1 font-medium">{pct}% of total revenue</p>
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                  <div>
                    <h2 className="text-sm font-bold text-gray-900">Today's Invoices</h2>
                    <p className="text-xs text-gray-400 font-medium mt-0.5">{todayInvoices.length} record{todayInvoices.length !== 1 ? "s" : ""} today</p>
                  </div>
                ...
                {todayInvoices.length === 0 ? (
                  <div className="text-center py-10 text-gray-400">
                    ...
                    <p className="text-sm font-medium">No invoices today yet</p>
                  </div>
                ) : (
                  <div className="divide-y divide-gray-50 max-h-72 overflow-y-auto">
                    {todayInvoices.map((inv) => (
                      <div key={inv._id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50/70 transition-colors">
