...
function generateInvoicePDF(inv, formatTime) {
  ...
  const startTime = inv.startTime
    ? new Date(inv.startTime).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })
    : null;
  const endTime = inv.endTime
    ? new Date(inv.endTime).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" })
    : null;
...
export default function Invoice() {
  ...
  const validateManual = () => {
    ...
    if (!manualForm.amount || Number(manualForm.amount) <= 0) e.amount = "Enter a valid amount";
    setManualErrors(e);
  ...
  <div className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-950 tracking-tight">Invoices</h1>
            <p className="text-sm text-gray-600 mt-0.5 font-medium">All billing records</p>
          </div>
          ...
          {filteredInvoices.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              ...
              <p className="text-sm font-medium">
                {search ? `No invoices found for "${search}"` : 'No invoices yet. Click "New Invoice" to create one.'}
              </p>
            </div>
          ) : (
            filteredInvoices.map((inv) => (
              ...
              >
                ...
                <div className="col-span-2 min-w-0 pr-4">
                  <p className="font-bold text-gray-900 text-sm truncate">{inv.name}</p>
                  <p className="text-xs text-gray-500 font-medium truncate">{inv.email}</p>
                </div>

                {/* Duration */}
                <p className="text-sm text-gray-800 font-semibold">{formatTime(inv.duration)}</p>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {inv.startTime && (
                      <p className="text-xs text-gray-400 whitespace-nowrap">
                        {new Date(inv.startTime).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}
                        {inv.endTime && (
                          <> → {new Date(inv.endTime).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</>
                        )}
                      </p>
                    )}
                  </div>
          ...
          >
            ...
            <div className="bg-[#1a4a3a] px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-base font-semibold text-white">New Manual Invoice</h2>
              </div>
            ...
            <div className="p-6 space-y-5">
              {/* Client Info */}
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Client Info</p>
                <div className="space-y-3">
                  <div>
                    ...
                    />
                    {manualErrors.name && <p className="text-red-500 text-xs mt-1">{manualErrors.name}</p>}
                  </div>
                  <div>
                    <input
                    ...
                    />
                    {manualErrors.email && <p className="text-red-500 text-xs mt-1">{manualErrors.email}</p>}
                  </div>
                  <input
              ...
              {/* Billing Details */}
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Billing Details</p>
                <div className="space-y-3">
                  <div>
                    ...
                    {manualErrors.amount && <p className="text-red-500 text-xs mt-1">{manualErrors.amount}</p>}
                  </div>
                  <textarea
